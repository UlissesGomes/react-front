...
export default class Menu extends Component {
  ...
  save() {
    ...
    const url = item._id ? `${URL}/${item._id}` : URL
      ...
      return(
        <tr key={item._id}>
          ...
          <td>
            ...
            <label> -- </label>
            <button className="btn btn-danger ml-5" onClick={ () => this.remove(item) }>
    ...
    return (
      <div role="form" className="itemForm">
        <Grid cols='12 9 10'>
          ...
          <input id="name" name="name" value={this.state.item.name} onChange={e => this.updateField(e)}
            type="text" placeholder="Nome do produto" className="form-control" />

          <h4>Descrição</h4>
          <input id="description" name="description" onChange={e => this.updateField(e)}
            value={ this.state.item.description}
            type="text" placeholder="Descrição do produto" className="form-control" />

          <h4>Valor em dinheiro</h4>
          <input id="priceCash" name="priceCash" onChange={e => this.updateField(e)}
            value={this.state.item.priceCash }
            type="number" placeholder="Valor em dinheiro" className="form-control" />

          <h4>Valor em débito</h4>
          <input id="priceDebit" name="priceDebit" onChange={e => this.updateField(e)}
        ...
        <Grid cols='12 9 10'>
          ...
          <select className="form-control" name="type" onChange={e => this.updateField(e)}  >
            <option value=""></option>
  ...
  render() {
    return (
      <div>
        <PageHeader name="Cardápio" small="Cadastro"></PageHeader>
        {this.renderForm()}
        <PageHeader name="Cardápio" small="Lista"></PageHeader>
        {this.renderTable()}
      </div>
    )
  }
